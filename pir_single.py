...
def on_event(gpio_pin):
    ...
    if current_state:
        ...
        if latest_intruder_left:
            print('state changed to HIGH at', get_formatted_timestamp(), 'after', get_formatted_time_difference(latest_intruder_left))
            # print('Intruder returned after', get_formatted_time_difference(latest_intruder_left), 'at', get_formatted_timestamp())
            latest_intruder_left = None
        else:
            print('state changed to HIGH at', get_formatted_timestamp())
        ...
        print('state changed to LOW  at', get_formatted_timestamp())
...
def main_true_loop():
    ...
    previous_state = GPIO.input(SENSOR_PIN)
    set_power_plug_state('on' if previous_state == 1 else 'off') # set initial state
    ...
    while True:
        ...
        if current_state == 0 and latest_intruder_left and (datetime.now() - latest_intruder_left).total_seconds() > TURN_OFF_AFTER_SECONDS:
            global power_plug_turned_on
            if power_plug_turned_on:
...
def main():
    print('Startup:', get_formatted_timestamp(), 'no one here!' if GPIO.input(SENSOR_PIN) == 0 else 'someone is here!')
